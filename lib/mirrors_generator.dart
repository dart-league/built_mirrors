...
String _combine(String pv, String e) => pv.isNotEmpty && e.isNotEmpty
    ? pv + ',' + e
    : pv.isEmpty
        ? e
        : e.isEmpty
...
String _renderFunction(ExecutableElement f) => "FunctionMirror("
...
String _renderNamedParameters(List<ParameterElement> params) {
  var result = params
      .where((p) => p.isNamed)
      .map((p) => "'${p.name}': ${_renderParameter(p)}")
      .join(',');
...
String _renderPositionalParameters(List<ParameterElement> params) {
  var result =
      params.where((p) => p.isPositional).map(_renderParameter).join(',');
  return result.isEmpty ? '' : 'positionalParameters: [$result],';
}

String _renderParameter(ParameterElement p) => "DeclarationMirror("
    "name: '${_getSerializedNameFromParameter(p)}',"
    'type: ${_renderType(p.type)}'
    '${p.isRequired ? ', isRequired: true' : ''}'
...
String _getSerializedNameFromAccessor(PropertyAccessorElement a) =>
    _getSerializedNameFromField(
        (a.enclosingElement as ClassElement).getField(a.displayName)!);

String _getSerializedNameFromParameter(ParameterElement p) {
  var field;
  if (p.enclosingElement is ConstructorElement) {
    field = (p.enclosingElement?.enclosingElement as ClassElement)
        .getField(p.displayName);
  }
  return field == null ? p.displayName : _getSerializedNameFromField(field);
}

String _renderMetadata(List<ElementAnnotation> metadata) {
  var annotations = metadata.where((a) => (a
          .computeConstantValue()
          ?.type
          ?.element as ClassElement)
      .allSupertypes
      .any(
          (st) => st.getDisplayString(withNullability: false) == 'Annotation'));
  return annotations.isNotEmpty
      ? "annotations: [${annotations.map((a) => a.element is ConstructorElement ? '${a.computeConstantValue()?.type?.getDisplayString(withNullability: false)}${_renderAnnotationParameters(a)}' : a.element?.name).join(',')}]"
      : '';
}

String _renderAnnotationParameters(ElementAnnotation annotation) {
  return (annotation.element as ConstructorElement)
      .parameters
      .map((p) => p.isNamed
          ? '${p.name}: ${_renderParameterValue(p, annotation)}'
          : _renderParameterValue(p, annotation))
      .toString();
}

String _renderParameterValue(
    ParameterElement parameter, ElementAnnotation annotation) {
  var thingy =
      ConstantReader(annotation.computeConstantValue()).read(parameter.name);
...
String? _renderValue(DartObject field) {
  ...
  if (cr.isList) {
    return '[${cr.listValue.map((v) => _renderValue(v)).join(',')}]';
  }
  if (cr.isMap) {
    var values = [];
    cr.mapValue.forEach(
        (k, v) => values.add("${_renderValue(k!)}: ${_renderValue(v!)}"));
    return '{${values.join(',')}}';
  }
  ...
  revived.namedArguments
      .forEach((k, v) => arguments.add('$k: ${_renderValue(v)}'));
  return '${field.type?.getDisplayString(withNullability: false)}(${arguments.join(',')})';
}

String _renderType(DartType type) =>
    type is ParameterizedType && type.typeArguments.isNotEmpty
    ? '[${type.element?.name}, ${_renderTypes(type.typeArguments)}]'
...
String _renderTypes(List<DartType> types) => types.length > 1
